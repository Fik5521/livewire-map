<div id="data-industri" class="bg-white min-vh-100 p-4">
    <div class="d-flex justify-content-between align-items-end pb-4 mb-4 border-bottom border-light-subtle">
        ...
        <div class="d-flex gap-2">
            ...
            <button wire:click="exportCsv" class="btn btn-white border px-4 py-2 rounded-3 fw-semibold text-dark shadow-sm">
            ...
            <div class="row align-items-center">
                <div class="col">
                    <div class="d-flex align-items-center">
                        ...
                        <select class="form-select form-select-sm border-light-subtle" style="width: 75px;" wire:model.live="perPage">
                ...
                <div class="col-md-4">
                    <div class="input-group">
                        ...
                        <input type="text" class="form-control border-start-0 border-light-subtle ps-0" placeholder="Cari nama, kecamatan, atau alamat..." wire:model.live="search">
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="table-responsive">
            <table class="table table-hover mb-0 align-middle">
                <thead class="bg-light-subtle border-bottom">
                    <tr>
                        <th class="ps-4 py-3 text-muted fw-semibold small text-uppercase" style="cursor: pointer;" wire:click="sortBy('name')">
                            Nama Industri <i class="bi {{ $sortField === 'name' ? ($sortDirection === 'asc' ? 'bi-sort-up' : 'bi-sort-down') : 'bi-arrow-down-up' }} ms-1"></i>
                        </th>
                        <th class="py-3 text-muted fw-semibold small text-uppercase" style="cursor: pointer;" wire:click="sortBy('kecamatan')">
                            Kecamatan <i class="bi {{ $sortField === 'kecamatan' ? ($sortDirection === 'asc' ? 'bi-sort-up' : 'bi-sort-down') : 'bi-arrow-down-up' }} ms-1"></i>
                        </th>
                ...
                <tbody class="border-top-0">
                    ...
                    <tr>
                        <td class="ps-4 py-3">
                            ...
                            <small class="text-muted text-uppercase" style="font-size: 10px;">{{ $loc->fcode }}</small>
                        </td>
                        <td class="py-3">
                        ...
                        <td class="pe-4 py-3 text-center">
                            <div class="d-flex gap-2 justify-content-center">
                                <button wire:click="edit({{ $loc->id }})" class="btn btn-sm btn-outline-warning" title="Edit" data-bs-toggle="tooltip">
                                    <i class="bi bi-pencil-square"></i> Edit
                                </button>
                                <button onclick="confirmDelete({{ $loc->id }})" class="btn btn-sm btn-outline-danger" title="Hapus" data-bs-toggle="tooltip">
                    ...
                    <tr>
                        <td colspan="4" class="text-center py-5">
                            <img src="https://illustrations.popsy.co/white/data-analysis.svg" alt="No data" style="width: 150px;" class="mb-3">
    ...
    <div class="modal fade @if($showModal) show @endif" style="display: @if($showModal) block @else none @endif; background: rgba(255,255,255,0.85); backdrop-filter: blur(4px);" tabindex="-1">
        <div class="modal-dialog modal-xl modal-dialog-centered">
            <div class="modal-content border border-light-subtle shadow-lg rounded-4 overflow-hidden">
                <div class="modal-header px-4 py-3 bg-white border-bottom border-light-subtle">
                    <div>
                        <h5 class="modal-title fw-bold text-dark mb-0">{{ $isEdit ? 'Sunting Data Industri' : 'Tambah Industri Baru' }}</h5>
                    ...
                    <div class="row g-4">
                        <div class="col-md-4">
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control border-light-subtle" wire:model="name" placeholder="Masukkan nama resmi...">
                            ...
                            <div class="mb-3">
                                ...
                                <div class="input-group">
                                    <span class="input-group-text bg-light border-light-subtle"><i class="bi bi-geo"></i></span>
                                    <input type="text" class="form-control bg-light border-light-subtle fw-bold" wire:model="kecamatan" readonly>
                                </div>
                            </div>
                            <div class="mb-3">
                            ...
                            <div class="p-3 bg-light rounded-3 border border-light-subtle">
                                <label class="form-label small fw-bold text-secondary d-block mb-2">KOORDINAT SPASIAL</label>
                                <div class="row g-2">
                                    <div class="col-6">
                                        <label class="text-muted" style="font-size: 10px;">LATITUDE</label>
                                        <input type="text" class="form-control form-control-sm bg-white border-light-subtle" wire:model="lat" readonly>
                                    </div>
                                    <div class="col-6">
                                        <label class="text-muted" style="font-size: 10px;">LONGITUDE</label>
                                        <input type="text" class="form-control form-control-sm bg-white border-light-subtle" wire:model="lng" readonly>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-8">
                            <div class="d-flex justify-content-between align-items-center mb-2">
                                <label class="form-label small fw-bold text-secondary mb-0">PENENTUAN LOKASI (INTERAKTIF)</label>
                                <span class="badge bg-warning-subtle text-warning border border-warning-subtle rounded-pill">Klik pada peta</span>
                            </div>
                            <div id="map-modal" style="height: 420px; border-radius: 12px; border: 1px solid #dee2e6;" wire:ignore></div>
                        </div>
                    </div>
                </div>
                <div class="modal-footer px-4 py-3 bg-light-subtle border-top border-light-subtle">
                    <button type="button" class="btn btn-link text-decoration-none text-muted fw-semibold me-auto" wire:click="closeModal">Batalkan</button>
                    <button type="button" wire:click="{{ $isEdit ? 'update' : 'save' }}" class="btn btn-primary px-5 py-2 rounded-3 fw-bold">
                    ...
                    modalMap.on('click', (e) => {
                        ...
                        if (geoJsonData) {
                            turf.featureEach(geoJsonData, (f) => {
                                if (turf.booleanPointInPolygon(pt, f)) detectedKec = f.properties.nm_kecamatan;
                            });
                        }
